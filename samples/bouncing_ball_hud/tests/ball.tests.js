...
QUnit.module("Ball rendering", (hooks) => {
  ...
  QUnit.test("reset() keeps the control synced to the new position and rotation", (assert) => {
    const ui = new FakeUi();
    const ball = new Ball({ ui, vx: 100, vy: 0, radius: 10 });
    ...
    assert.strictEqual(ball.control.left, 20);
    assert.strictEqual(ball.control.top, -10);
    assert.strictEqual(ball.control.rotation, 0);
  });
