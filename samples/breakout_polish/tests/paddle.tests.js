QUnit.module("Paddle", () => {
  ...
  QUnit.test("setHalfWidth() updates halfWidth (no mesh to sync without a scene)", (assert) => {
    const paddle = new Paddle({ halfWidth: 1 });

    paddle.setHalfWidth(2);

    assert.strictEqual(paddle.halfWidth, 2);
  });

  QUnit.test("setHalfWidth() changes what moveTo() clamps against", (assert) => {
    const paddle = new Paddle({ halfWidth: 1 });
    paddle.setHalfWidth(3);

    paddle.moveTo(100, 10);

    assert.strictEqual(paddle.x, 7, "clamped to boundHalfWidth - the new halfWidth");
  });
});
...
QUnit.module("Paddle rendering", (hooks) => {
  ...
  QUnit.test("setHalfWidth() scales mesh.scaling.x relative to the geometry's baked-in half-width", (assert) => {
    const scene = {};
    const paddle = new Paddle({ scene, halfWidth: 1 });

    paddle.setHalfWidth(2.5);

    assert.strictEqual(paddle.mesh.scaling.x, 2.5, "2.5 / 1 (the half-width attachTo() built the box at)");
  });
