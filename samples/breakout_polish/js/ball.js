// Ball state AND rendering, carried over from Breakout Bricks unchanged
// except for one addition: update()'s bounds now accepts an `openBottom`
// flag, which this sample uses so a missed ball falls through instead of
// bouncing -- see the comment on that check below. bounceOffPaddle(),
// bounceOffBrick(), restitution/rest handling, and mesh sync are untouched.
//
...
class Ball {
  ...
  //
  // If `bounds.openBottom` is true, the bottom edge doesn't bounce at all --
  // the ball is left to keep falling past it uncorrected, and it's the
  // caller's job to notice (this sample uses that to detect a missed ball
  // and take a life, instead of the ball bouncing forever like every earlier
  // sample in this series).
  update(dt, bounds) {
    const { halfWidth, halfHeight, openBottom = false } = bounds;
    const events = [];
    ...
    if (this.y + this.radius > halfHeight && !openBottom) {
  ...
  // Checks for a collision against an axis-aligned brick ({x, y, halfWidth,
  // halfHeight}) and, if hit, bounces off whichever side was hit. Unlike
  // bounceOffPaddle (always a from-above hit with steering), a brick can be
  // hit from any side, so this picks the reflection axis by comparing
  // overlap depth on x vs y -- whichever axis the ball has penetrated less
  // is the side it actually crossed, so that's the one that reflects. The
  // ball is also nudged out of the brick by that overlap so it doesn't stay
  // stuck inside it (and re-trigger the same collision) on the next frame.
  // Returns a bounce event in the same shape as a wall/paddle bounce (or
  // null if there's no overlap); brick destruction/scoring is the caller's
  // job (see BrickGrid.checkCollision in ../js/bricks.js).
  bounceOffBrick(brick) {
    const dx = this.x - brick.x;
    const dy = this.y - brick.y;
    const overlapX = this.radius + brick.halfWidth - Math.abs(dx);
    const overlapY = this.radius + brick.halfHeight - Math.abs(dy);
    if (overlapX <= 0 || overlapY <= 0) return null;

    if (overlapX < overlapY) {
      const sign = dx >= 0 ? 1 : -1;
      this.x += sign * overlapX;
      this.vx = Math.abs(this.vx) * sign;
      this._syncMesh();
      return this._bounceEvent(this.x, this.y, sign, 0);
    }

    const sign = dy >= 0 ? 1 : -1;
    this.y += sign * overlapY;
    this.vy = Math.abs(this.vy) * sign;
    this._syncMesh();
    return this._bounceEvent(this.x, this.y, 0, sign);
  }
