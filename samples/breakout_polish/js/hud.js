// On-screen score + lives + level + reset button, plus four centered
// overlays (Start, Paused, Game Over, and You Win), following the same
...
// center-origin coordinate space the ball and sparks move in. The overlay
// labels are the exception: they're added straight to `ui` rather than the
// top-pinned panel, so they keep `BABYLON.GUI`'s default center/center
// alignment and show up in the middle of the screen.
class Hud {
  ...
  // and level labels plus a reset button, and four separate centered overlay
  ...
  attachTo(ui) {
    ...
    this.startLabel = this._createOverlayLabel("Click or tap to start", "white");
    this.startLabel.isVisible = true; // the only overlay shown before the first click
    this.pauseLabel = this._createOverlayLabel("Paused", "#8ab4ff");
  ...
  dispose() {
    ...
    this.panel.dispose();
    for (const label of [this.startLabel, this.pauseLabel, this.gameOverLabel, this.winLabel]) {
  ...
  // Adds one life without affecting Game Over state (used by the "life"
  // power-up -- unlike resetLives(), this never runs while the game is
  // actually over, so there's no overlay to hide).
  addLife() {
    this.lives += 1;
    this._syncLabels();
  }
  ...
  showStart() {
    if (this.startLabel) this.startLabel.isVisible = true;
  }

  hideStart() {
    if (this.startLabel) this.startLabel.isVisible = false;
  }

  showPause() {
    if (this.pauseLabel) this.pauseLabel.isVisible = true;
  }

  hidePause() {
    if (this.pauseLabel) this.pauseLabel.isVisible = false;
  }
