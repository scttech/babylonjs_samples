...
const createScene = () => {
  ...
  scene.onBeforeRenderObservable.add(() => {
    ...
    for (const bounce of bounces) {
      // Sparks are real mesh-space particles now (see sparks.js), so the
      // bounce point/normal only need the same y-flip Ball._syncMesh applies
      // -- no world<->screen conversion, unlike the GUI-space indicator.
