...
QUnit.module("Ball open bottom", () => {
  QUnit.test("falls through the bottom uncorrected when openBottom is true", (assert) => {
    const ball = new Ball({ x: 0, y: 90, vx: 0, vy: 100, radius: 10, restitution: 1 });
    const bounds = { halfWidth: 100, halfHeight: 100, openBottom: true };

    const events = ball.update(1, bounds);

    assert.strictEqual(ball.y, 190, "y is left to advance past halfHeight uncorrected");
    assert.strictEqual(ball.vy, 100, "vy is untouched -- no bounce happens");
    assert.deepEqual(events, [], "no bounce event is reported for the open bottom");
  });

  QUnit.test("the top wall still bounces normally when openBottom is true", (assert) => {
    const ball = new Ball({ x: 0, y: -90, vx: 0, vy: -100, radius: 10, restitution: 1 });
    const bounds = { halfWidth: 100, halfHeight: 100, openBottom: true };

    const events = ball.update(1, bounds);

    assert.strictEqual(ball.y, -90, "top wall still clamps as usual");
    assert.strictEqual(ball.vy, 100, "vy is still reflected off the top");
    assert.strictEqual(events.length, 1);
    assert.deepEqual(events[0], { x: 0, y: -100, normalX: 0, normalY: 1, speed: 100 });
  });

  QUnit.test("left/right walls still bounce normally when openBottom is true", (assert) => {
    const ball = new Ball({ x: 90, y: 0, vx: 100, vy: 0, radius: 10, restitution: 1 });
    const bounds = { halfWidth: 100, halfHeight: 100, openBottom: true };

    const events = ball.update(1, bounds);

    assert.strictEqual(ball.vx, -100, "vx is still reflected off the right wall");
    assert.strictEqual(events.length, 1);
  });

  QUnit.test("defaults to openBottom: false (bottom still bounces) when omitted", (assert) => {
    const ball = new Ball({ x: 0, y: 90, vx: 0, vy: 100, radius: 10, restitution: 1 });
    const bounds = { halfWidth: 100, halfHeight: 100 };

    const events = ball.update(1, bounds);

    assert.strictEqual(ball.vy, -100, "bottom wall still bounces when openBottom is omitted");
    assert.strictEqual(events.length, 1);
  });
});
