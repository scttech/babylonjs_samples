QUnit.module("Hud counting", () => {
  ...
  QUnit.test("showGameOver()/hideGameOver()/showWin()/hideWin() are no-ops when ui is omitted", (assert) => {
    const hud = new Hud();
...
QUnit.module("Hud rendering", (hooks) => {
  ...
  QUnit.test("constructor adds hidden Game Over and Win labels directly to ui, not the panel", (assert) => {
    const ui = new FakeUi();
    const hud = new Hud({ ui });
  ...
  QUnit.test("dispose() removes and disposes the panel and both overlay labels", (assert) => {
    const ui = new FakeUi();
    const hud = new Hud({ ui });
    const panel = hud.panel;
    const gameOverLabel = hud.gameOverLabel;
    const winLabel = hud.winLabel;
    ...
    assert.false(ui.controls.includes(winLabel), "win label removed from the ui");
    assert.true(winLabel.disposed, "win label.dispose() was called");
    assert.strictEqual(hud.panel, null);
    assert.strictEqual(hud.gameOverLabel, null);
    assert.strictEqual(hud.winLabel, null);
  });
